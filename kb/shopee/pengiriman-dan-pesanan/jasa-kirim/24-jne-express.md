---
id: shp-pengiriman-024
title: JNE Express
platform: shopee
kategori: pengiriman-dan-pesanan/jasa-kirim
depth: 2
status: canonical
confidence: tinggi
sensitif_waktu: true
valid_as_of: 2026-08
sources:
  - file: pp---jasa-kirim/jne-express.md
    bagian: "artikel penuh"
related: [shp-pengiriman-012]
---
# JNE Express

## Ringkasan
JNE Express, 4 kategori layanan: **Reguler** (JNE Reguler Cashless — ongkir dipotong dari pelepasan dana; JNE Reguler Non-Cashless/JOB — bayar online, khusus Penjual terpilih), **Hemat Kargo** (JNE Trucking/JTR — Cashless, mulai 3kg, dihitung dari 5kg, per kg mulai 6kg, biaya tambahan kalau >250kg), **Next Day** (JNE YES — tiba besok termasuk Minggu/libur nasional, cut-off jam 16.00 WIB). Kriteria pick-up (Reguler & YES) dievaluasi bulanan berdasar rata-rata pesanan bersih harian, sama seperti jasa kirim lain — dihapus kalau gak penuhi syarat 1 bulan berturut-turut. Paket gagal kirim: via pick-up balik ke Alamat Pengembalian; via drop-off balik ke Agen/Counter asal (7 hari kalender buat ambil, lewat itu JNE berhak musnahkan). 6 kategori produk dilarang.

## Kapan ini dipakai
Dipakai kalau member nanya jenis layanan, kriteria pick-up, jadwal, atau kendala JNE Express. Buat FAQ teknis JNE (JOB vs Reguler, resi salah, dll.), arahkan ke `shp-pengiriman-012`.

## Isi

**Jenis Layanan Jasa Kirim JNE Express**



  - **Reguler**

**1. JNE Reguler**

Layanan jasa kirim JNE Express secara *cashless* (non-tunai), di mana Penjual tidak perlu membayar biaya ongkos kirim kepada kurir karena biaya ongkos kirim tersebut akan dipotong dari total pelepasan dana secara otomatis. JNE Reguler tersedia untuk Penjual di seluruh Indonesia.

Keuntungan menggunakan layanan JNE Reguler:

  - Tidak perlu bayar ongkos kirim secara tunai ke kurir/Agen
  - Proses pengiriman lebih cepat tanpa input resi secara manual
  - Pilihan *pick up* atau antar ke counter (*drop off*)
  - Status pesanan otomatis ter-*update* setelah Agen JNE Express melakukan *scan* no. resi

  

**2. JNE Reguler (Non-Cashless)**

Layanan jasa kirim JNE Express di mana Anda dapat mengisi no. resi pengiriman dan melakukan pembayaran ongkos kirim secara *online*. Anda dapat menghemat waktu dengan menggunakan JNE JOB (Online Booking). Anda hanya perlu menunjukan kode booking yang didapatkan secara *online* kepada agen di counter JNE Express untuk mengirim pesanan. Saat ini, jasa kirim JNE Reguler (Non-Cashless) hanya dapat dinikmati oleh Penjual yang memenuhi kriteria.

  

  - **Hemat Kargo**

**1. JNE Trucking (JTR)**

Layanan JNE Express secara *cashless* (non-tunai) yang mendukung pengiriman dalam jumlah besar menggunakan armada truk melalui darat & laut dengan harga yang kompetitif. Layanan ini berlaku untuk pengiriman nasional. JNE Trucking (JTR) melayani pengiriman mulai dari 3kg dan biaya pengiriman akan terhitung dari berat paket 5kg. Sedangkan biaya pengiriman untuk berat paket mulai 6kg akan dihitung per kg.

  

  - **Next Day**

**1. JNE YES**

Dengan layanan ini, paket Anda akan sampai tujuan keesokan harinya (termasuk hari Minggu & libur nasional) dengan perkiraan durasi pengiriman 1 hari, disesuaikan pada daerah tujuan pengiriman.

  

**Layanan ini tersedia di berbagai wilayah sesuai dengan area jangkauan yang telah ditentukan.**

  

**Berikut** **detail** **ketentuan menggunakan jasa kirim JNE Express:**

**⚠️Catatan**

  - Pastikan Anda telah[ mengaktifkan jasa kirim JNE Express](https://seller.shopee.co.id/edu/article/12518/Mengatur-Jasa-Kirim) pada pengaturan toko dan setiap produk agar Pembeli dapat memilih layanan pengiriman Hemat Kargo. 
  - Pastikan Anda telah memahami[ Alur Pengiriman dengan Jasa Kirim yang Didukung Shopee](https://seller.shopee.co.id/edu/article/14180) sebelum melakukan pengiriman pesanan.
  - Setiap layanan pengiriman dipastikan mengikuti kriteria[ Batasan Berat dan Ukuran Paket untuk Pengiriman di Shopee](https://seller.shopee.co.id/edu/article/16188).
  - Apabila biaya ongkos kirim Jasa kirim JNE Trucking (JTR) lebih dari 250kg maka akan terdapat tambahan biaya. Pelajari lebih lanjut tentang[ tambahan biaya pengiriman JNE Trucking (JTR)](https://seller.shopee.co.id/edu/article/17835).

**  
**Area Jangkauan Penjual JNE Express**

**Area Jangkauan JNE Reguler**

  

  

  

  

**Area Jangkauan JNE Trucking (JTR)**

  

  

  

**Area Jangkauan JNE YES**

  
  
  

  

  

**Kriteria Layanan Pick up JNE Express**

Untuk menjaga konsistensi layanan *pick up* pada pengiriman JNE Reguler & JNE YES, berikut kriteria layanan *pick up* yang berlaku bagi Penjual.

Cara hitung rata-rata pesanan harian JNE Reguler & JNE YES: 

📍 **Definisi**

Jumlah pesanan bersih adalah jumlah pesanan terbayar yang tidak termasuk:

  - Pesanan dibatalkan
  - Pesanan yang dikembalikan karena pengiriman gagal atau pengembalian barang/dana

  

**Ketentuan Layanan** **Pick up** **JNE Express**

Performa pesanan harian di toko Anda akan dievaluasi setiap bulan. Jika toko Anda belum memiliki layanan *pick up* dan telah memenuhi kriteria yang ditentukan, maka layanan *pick up* akan otomatis tersedia pada akun Anda di **minggu pertama bulan berikutnya**.

  

Contoh:

Namun, jika Anda tidak memenuhi kriteria rata-rata pesanan bersih harian selama 1 bulan berturut-turut, maka layanan *pick up* akan ditinjau kembali dan dapat dinonaktifkan.

  

Contoh:

**⚠️ Catatan**

  - Kriteria layanan *pick up* JNE Reguler & JNE YES dapat berubah sewaktu-waktu sesuai kebijakan JNE Express. Pelajari lebih lanjut mengenai[ proses *pick up*](https://seller.shopee.co.id/edu/article/12850) pada pengiriman pesanan.
  - Jika layanan *pick up* dinonaktifkan, Anda tetap dapat mengirimkan paket dengan cara *drop off*/antar ke *counter* JNE Express terdekat.

  

**Jadwal Operasional Pick up dan Drop off Paket JNE Express**

  

**JNE Reguler dan JNE Trucking (JTR)**

  

**JNE Next Day** 

  

**⚠️ Catatan**

Batas waktu pengiriman (*cut-off time*) berlaku untuk pengiriman dengan JNE YES saat paket diserahkan ke kurir. Batas waktu pengirimannya adalah pk 16.00 WIB agar dapat terkirim ke Pembeli di esok harinya.

  

**Produk yang Tidak Diperbolehkan Dikirim dengan JNE Express**

Berikut merupakan produk-produk yang tidak diperbolehkan dikirim dengan jasa kirim JNE Express:

1.  Barang mudah meledak & terbakar
2.  Senjata api/tajam
3.  Narkotika, psikotropika, dan obat-obat terlarang lainnya
4.  Emas, perangko, uang asli dan uang palsu, cek/giro
5.  Barang yang melanggar kesusilaan
6.  Barang yang menurut undang-undang dinyatakan sebagai barang terlarang

  

⚠️ **Catatan**

  - Pastikan Anda tidak menjual[ Produk yang Dilarang dan/atau Dibatasi sesuai kebijakan Shopee](https://seller.shopee.co.id/edu/article/6843).
  - Shopee berhak sewaktu-waktu mengubah atau menambahkan daftar produk yang tidak diperbolehkan dikirim sesuai kebijakan dari JNE Express.

  

**Kendala Seputar Jasa Kirim JNE Express**

⚠️**Catatan**

  - Jika terjadi kegagalan pengiriman ke alamat Pembeli, maka Anda akan menerima notifikasi "Pengiriman gagal" dan paket akan dikembalikan ke Penjual berdasarkan metode pengiriman yang dipilih saat mengatur pengiriman:
      
      - *Pick up*: Dikirim ke[ Alamat Pengembalian](https://seller.shopee.co.id/edu/article/26082) yang telah Anda atur.
      - *Drop off*: Dikirim ke Agen/*Counter* yang sama dengan tempat Anda menyerahkan paket. Jika barang yang dikembalikan telah sampai di lokasi sortir, Anda akan menerima notifikasi "Pesanan telah dikembalikan" untuk mengambil paket dalam waktu 7 hari kalender. Jika melewati batas waktu, maka JNE Express berhak untuk mengambil tindakan terhadap paket (termasuk memusnahkannya).
  - Untuk memastikan produk Anda sampai ke Pembeli dengan aman dan dalam kondisi yang baik, pastikan Anda menggunakan bahan dan teknik yang tepat saat mengemas paket. Pastikan kemasan paket sesuai dengan jenis atau kategori produk. Pelajari lebih lanjut terkait[ Panduan Pengemasan Produk di Shopee](https://seller.shopee.co.id/edu/article/18821).
  - Pengajuan klaim wajib melampirkan KTP yang terlihat jelas dan tidak terpotong. Jika KTP tidak disertakan atau tidak sesuai, Shopee akan menghubungi Anda melalui email info.jasakirim@shopee.co.id untuk tindak lanjut.
  - Berikan perlindungan terhadap produk yang Anda kirim dari kehilangan/kerusakan selama pengiriman. Pelajari lebih lanjut tentang[ Asuransi Pengiriman Shopee](https://seller.shopee.co.id/edu/article/17345).
  - Untuk mendapatkan informasi seputar pengiriman pesanan atau kendala dalam proses pengiriman, Anda dapat[ Menghubungi Pihak Jasa Kirim](https://seller.shopee.co.id/edu/article/14227).
  - Apabila terdapat pengajuan pengembalian barang/dana, pelajari lebih lanjut terkait[ FAQ Pengembalian Barang/Dana](https://seller.shopee.co.id/edu/article/17055/faq-pengembalian-barang-dan-dana).

## Angka & patokan

| Patokan | Nilai |
|---|---|
| Berat minimum layanan JTR | mulai 3kg |
| Berat dasar hitung biaya JTR | dari 5kg |
| Perhitungan biaya JTR di atas | per kg, mulai 6kg |
| Ambang biaya tambahan JTR | >250kg |
| Estimasi durasi JNE YES | 1 hari (termasuk Minggu & libur nasional) |
| Batas waktu serah terima (cut-off) JNE YES | pk. 16.00 WIB |
| Batas ambil paket gagal (drop-off) | 7 hari kalender sejak notifikasi "Pesanan telah dikembalikan" |
| Jumlah kategori produk dilarang | 6 kategori |

## Pertanyaan diagnosa

1. **Member kirim paket JTR 2kg, bisa gak?** GAK BISA — JNE Trucking (JTR) layani pengiriman minimum mulai 3kg.
2. **Member kirim JTR 4kg, dihitung ongkirnya sebagai berapa kg?** Dihitung sebagai 5kg (berat dasar minimum perhitungan biaya JTR), meski beratnya cuma 4kg.
3. **Member serahkan paket JNE YES ke kurir jam 17.00 WIB, apa tetap sampai besok?** Berisiko TELAT — cut-off time JNE YES adalah pk. 16.00 WIB; lewat dari itu biasanya baru diproses di siklus berikutnya.
4. **Member drop-off paket JNE, gagal kirim, paket balik ke lokasi sortir, kapan batas ambilnya?** 7 hari kalender sejak notifikasi "Pesanan telah dikembalikan" — kalau lewat, JNE berhak ambil tindakan termasuk MEMUSNAHKAN paket.
5. **Member mau kirim paket via pick-up, gagal kirim, kemana paket baliknya?** Ke Alamat Pengembalian yang sudah diatur Penjual — beda dengan metode drop-off yang balik ke Agen/Counter asal.
6. **Member kirim emas pakai JNE Express?** GAK BISA — emas termasuk dalam 6 kategori produk yang dilarang dikirim via JNE Express (bersama uang asli/palsu, cek/giro, perangko).
